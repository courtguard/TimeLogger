...
bool MyDatabase::setDonutChartQuery(QDate firstDayOfWeek, QDate lastDayOfWeek)
{
    ...
    else
    {
        ...
        else
        {
            ...
            emit updateHours(getSum.value(0).toString());
            isOkay = true;
        }
...
bool MyDatabase::getProjectCountQuery()
{
    ...
    getSize.prepare("select count(projectname) from projects");
    if(!getSize.exec()){
...
bool MyDatabase::getProjectQuery(int flag)
{
    ...
    getProjects.prepare("select projectname from projects");
    if(!getProjects.exec())
    {
...
bool MyDatabase::createProjectsRowQuery(QString projectName, QString projectDescription, QString startDate, QString endDate, QString companyName, QString clientName)
{
    bool isOkay = false;
    QSqlQuery queryFillInTable(m_myDatabase);
    queryFillInTable.prepare("insert into projects values ('"+projectName+"','"+projectDescription+"','"+startDate+"','"+endDate+"','"+companyName+"','"+clientName+"');");
    if(queryFillInTable.exec())
    {
        isOkay = true;
    }
